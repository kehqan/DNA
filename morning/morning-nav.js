 ...
 *
 * What it does:
 *   1. Hides the existing white masthead (non-destructively — display:none)
 *   2. Injects a matching liquid glass nav pill at the top of the page
 *   3. Wires the 3 action buttons to the existing JS functions
 *      (mastheadSave / openFeedback / openSettings — already in morning/index.html)
 *   4. Moves body top padding so content doesn't jump under the new nav
 *
 * Zero changes to morning/index.html required.
...
(function () {
  ...
  /* ── STYLES ─────────────────────────────────────────── */
  const CSS = `
    /* Hide old masthead non-destructively */
    .masthead { display: none !important; }

    /* Push body content down so it clears the new sticky nav */
    ...
    #mb-nav-pill .mb-btn.saved {
      color: #4ADE80;
    }
    #mb-nav-pill .mb-btn svg {
      flex-shrink: 0;
      opacity: 0.7;
    }

    /* Responsive — sidebar / narrow */
  ...
  function buildNav() {
    ...
    <div id="mb-nav-wrap">
      <div id="mb-nav-pill">
        ...
        <!-- Save setup -->
        <button class="mb-btn" id="mb-save-btn">
          ${ICON.save}
          <span>Save setup</span>
        </button>

        <!-- Feedback -->
        <button class="mb-btn" id="mb-feedback-btn">
          ${ICON.feedback}
          <span>Feedback</span>
        </button>

        <!-- Settings -->
        <button class="mb-btn" id="mb-settings-btn">
          ${ICON.settings}
          <span>Settings</span>
        </button>
  ...
  function wireButtons() {
    // Save setup — mirrors mastheadSave() logic, then flashes button green
    const saveBtn = document.getElementById('mb-save-btn');
    if (saveBtn) {
      saveBtn.addEventListener('click', () => {
        // Call the existing function if available
        if (typeof mastheadSave === 'function') {
          mastheadSave();
        } else if (typeof saveDefault === 'function') {
          saveDefault();
        }
        // Visual flash on new button too
        const orig = saveBtn.innerHTML;
        saveBtn.classList.add('saved');
        saveBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 13 13" fill="none"><path d="M2 7l3 3 6-6" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Saved</span>`;
        setTimeout(() => {
    ...
    // Feedback — calls existing openFeedback()
    const fbBtn = document.getElementById('mb-feedback-btn');
    if (fbBtn) {
    ...
    // Settings — calls existing openSettings()
    const stBtn = document.getElementById('mb-settings-btn');
    if (stBtn) {
  ...
  function init() {
    ...
    // 2. Inject nav markup at top of body
